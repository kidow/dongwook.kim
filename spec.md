...
### Phase 11: `/erd-editor` — ERD 에디터

drawDB의 기본 사용 경험을 참고하여, 가상 화이트보드 위에서 데이터베이스 다이어그램(ERD)을 그릴 수 있는 편집기.

#### 11-1. 기본 레이아웃 및 캔버스

- [x] `app/erd-editor/page.tsx` 라우트 추가
- [x] 툴바/캔버스 2단 구성 레이아웃
- [x] 테이블 노드 추가/선택/이동 기본 상호작용

#### 11-2. 핵심 ERD 편집 기능 (MVP)

- [x] 테이블 생성/삭제, 테이블명 수정
- [x] 컬럼 추가/수정/삭제 (이름, 타입, nullable 최소 지원)
- [x] 관계선 연결/삭제 (1:1, 1:N 기본 관계)
- [x] 확대/축소, 팬 이동 등 캔버스 기본 제스처 지원

#### 11-3. 로컬 저장/복원

- [x] ERD 상태를 `localStorage`에 저장
- [x] 페이지 재진입 시 저장된 다이어그램 자동 복원
- [x] 초기화(Reset) 액션 및 저장 스키마 버전 키 관리

#### 11-4. 위젯 등록

- [x] `app/page.tsx` Side Projects 섹션에 `WidgetLink` 추가
- [x] 적절한 아이콘 선정 (lucide-react `DatabaseIcon` 등)

#### 11-5. 참고 오픈 소스

- [x] drawDB의 핵심 UX/동작 흐름 벤치마킹

**참고**: https://github.com/drawdb-io/drawdb

---

### Phase 12: 홈페이지 위젯 통합 및 최종 정리

#### 12-1. Side Projects 섹션 레이아웃 조정

- [x] 12개 위젯(기존 2 + 신규 10)에 맞는 그리드 레이아웃 검토
- [x] 위젯 순서 및 배치 최적화
- [x] 반응형 그리드 검증 (모바일/태블릿/데스크탑)

#### 12-2. 공통 정리

- [x] 미사용 의존성 정리
- [x] 번들 사이즈 분석 및 최적화 (대형 라이브러리: Excalidraw, Sandpack, React Flow)
- [x] `pnpm build` 전체 빌드 성공 확인
- [x] 각 라우트 수동 동작 검증
...
- [x] 기능 정상 동작 확인 (`pnpm dev`)
- [x] 반응형 레이아웃 검증
- [x] 홈페이지 위젯 등록 완료
- [x] Conventional Commit 작성
- [x] spec.md 갱신
...
**최종 업데이트**: 2026-02-17
**현재 Phase**: Phase 12 완료 → Phase 3 진행 예정 (`/url-shortner`)
